...
bool PlayState::onEnter() {
  ...
  jsonLoader.loadDataMain( commonObjectsData_, projectileObjectsData_ );
...
bool PlayState::loadLevelFromFile( int currentLevel ) {
  ...
  JsonLoader jsonLoader;
  jsonLoader.loadLevel( currentLevel, levelObjectsData_, projectileObjectsData_, backgroundFilename_, pointsNeeded_ );
...
void PlayState::spawnProjectile( int projectileType, Sprite* originSprite ) {
  ...
  for( unsigned int i = 0; i < projectileObjectsData_.size(); i++ ) {
    if(
      ( projectileType == PROJECTILEHEROBULLET && ( *projectileObjectsData_[i] ).objectTypeSub == "projectileHeroBullet" )
      ||
      ( projectileType == PROJECTILEENEMYBASIC && ( *projectileObjectsData_[i] ).objectTypeSub == "projectileEnemyBasic" )
      ||
      ( projectileType == PROJECTILEENEMYCHUCKER && ( *projectileObjectsData_[i] ).objectTypeSub == "projectileEnemyChucker" )
      ||
      ( projectileType == PROJECTILEHEROBOMB && ( *projectileObjectsData_[i] ).objectTypeSub == "projectileHeroBomb" )
    ) {
      newObjectData = ( *projectileObjectsData_[i] );
    }
  }
  
  //std::cout << "width is " << newObjectData.width << std::endl;
  
  if( projectileType == PROJECTILEHEROBULLET || projectileType == PROJECTILEHEROBOMB  ) { // generated by the hero
    newObjectData.x = hero_ -> getPosition().getSpriteCenter().getX();
    newObjectData.y = hero_ -> getPosition().getSpriteCenter().getY();
    ...
    newObjectData.x = originSprite -> getPosition().getCoordinates().getX();
    newObjectData.y = originSprite -> getPosition().getCoordinates().getY();
  ...
  std::unique_ptr<ObjectData> projectileObjectData ( new ObjectData( newObjectData ) );
  
  if( projectileType == PROJECTILEHEROBULLET ) {
    projectileData.speedFactor = 7.0f;
    projectileData.destroyAtDest = true;
    projectileObjectData -> objectType = "PBullet";
  } else if( projectileType == PROJECTILEHEROBOMB ) {
